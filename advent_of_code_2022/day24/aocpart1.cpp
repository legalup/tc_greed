...
vector<pii> getnbrs(pii z){
  auto [r,c] = z;
  vector<pii> ret;
  if(r>1) ret.push_back({r-1,c});
  if(c>1) ret.push_back({r,c-1});
...
void run(T & cc){
  ...
  int timeconstant = lcm(nrows-1,ncols-1);
  ...
  auto getkids = [&](pii start, set<pii>&kids)->int{
                   ...
                   for(int time=0; time<timeconstant; time++){
                     int mytime = (time+1) % timeconstant;
  ...
  for(int epoch = 0; epoch < 100; epoch++){
    set<pii> kids;
    for(auto pp : todo){
      int tt = getkids(pp,kids);
      if(tt >= 0){
        cout << "epoch is: " << epoch << endl;
        cout << "got an answer: " << epoch*timeconstant + tt << endl;
        return;
      };
    }
    todo.clear();

    forzin(kids) todo.push_back(z);
  };
  

  cout << "sorry, we timed out" << endl;
};
